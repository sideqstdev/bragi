...
const ErrorToast: React.FC<errorToastProps> = ({
  ...
  return (
    <div
      className={`flex justify-between items-center ${variants[variant]} bg-opacity-75 relative min-w-full transition-transform duration-300 rounded-md px-5 py-2`}
      style={{ backgroundColor: customColor }}
    >
      <div className={`flex flex-row items-center`}>
        <span className={`mr-3`}>{variantIcons[variant]}</span>
      ...
      <div className={`flex justify-end`}>
        <a
          onClick={onClose}
          className={`cursor-pointer hover:text-${theme}-disabled`}
        >
